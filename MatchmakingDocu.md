## **Amazon FlexMatch with AnywhereFleet, OdinFleet and UnrealEngine**

Requirements
* A dedicated Unreal Engine Gameserver with AWS Gamelift
* An Unreal Gameclient with AWS Gamelift
* A Backendservice as some kind of Webservice

If you need assistence with one of these, [here]() is a full guid to create these.
...
* Use matchmaking in the GameClinet
...
To start a Flexmatch search, we need to call a [StartMatchmakingCommand](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/client/gamelift/command/StartMatchmakingCommand/) and pass a unique id, the name of the used matchmaking configuration and the players that are trying to get matched.
...
If the client calls this, it returns an JSON-Object with a ticketId. That ticket id will be used to check the status of the matchmaking, we will get into this later on. 
...
Gamelift matches other clients that also called StartMatchmaking, if the matchmaking configuration and ruleset allow to get matched.
Clients check the ticketId for the status of the Matchmaking. If the matchmaking is successfull, it returns the connectioninfo with an ip-address and port.
To check the status [DescribeMatchmakingCommand](https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/client/gamelift/command/DescribeMatchmakingCommand/) can be used.
But this should only be used for tests since it should not be called continually. To keep track of the ticket status, AWS Notifications should be used.

## Setup AWS notification channel and subscriptions

AWS offers an [notification service](https://docs.aws.amazon.com/sdk-for-javascript/v2/developer-guide/sns-examples.html) to communicate between AWS products (like GameLift) and an webendpoint. This can and should be used for several GameLift commands. 

To use an notifications you need following:
* Create a notification topic
* Create the enpoint (https for this example)
* Add a subscription
...
```
To add the subscription, an empty endpoint is enough. You just need to know the correct URL.

### Add a subscription
After you created the topic you need to add a subscription.
![Add Subscription](CreateSubscription.jpg)  

The subscription defines which delivery protocol is used. AWS SNS supports different protocols (Email,SMS, HTTP, HTTPS and more). For this case we are using the HTTPS protocol. As enpoint for this subscription we use the previously created endpoint in our backend service.
...
In your empty endpoint implementation we added a log. AWS sends an body in following format:
```
{
Message: "You have chosen to subscribe to the topic arn:aws:sns:eu-central-1:452994616046:NewTopic.
...
Timestamp: "<timestamp>"
Token: "<token>"
TopicArn: "arn:aws:sns:<data>:NewTopic"
...
### Gamesession placement queue
When using matchmaking you can choose if either AWS handles the creation of a gamesession, if a match is found, or you can take care of that by yourself. In our case we want to let AWS handle this. For that we need a placement queue. AWS will put a gamesession request in that queue and when that request is on top of the queue and there is capacity on an server, a new gamesession will be created. You can also add a notification to that queue to get the updates on queued and created gamesessions. To create a new queue open the [dashboard](https://eu-central-1.console.aws.amazon.com/gameliftservers/queues) and click create queue.
...
Go to your [GameLift dashboard](https://eu-central-1.console.aws.amazon.com/gameliftservers/matchmaking-rule-sets) to Matchmaking rule sets and crete a new matchmaking rule set.
...
```
This rule set matches players which use the same game mode and have a skill with a maximum distance of 200. These attributes are developer custom and are passed by the gameclient to the StartMatchmakingCommand. These attributes are part of the 'Players' attributes in the input object of the command.
...
Now we need to add a configuration that uses this rule set. Go to [Matchmaking configurations](https://eu-central-1.console.aws.amazon.com/gameliftservers/matchmaking-configurations), create a new matchmaking configuration, give it a name, select the rule set you want to use and choose Managed as FlexMatch mode and continue. If you choose standalone you need to handle the creation of the gamesession by yourself.
If you chose Managed you need to specify a placement queue. If the matchmaking is successfull AWS will create a gamesession request and add it to that queue.
![Conifguration](NewConfig.jpg)

Now you need to specify the configuration settings. 
...
If you want players to accept the potentially found match you need to check the acceptance required flag. To keep it simple we don't do this.
...
The body of the notification is a JSON object in following structure.
```
{
   "version":"0",
   "id":"<id>",
...
* MatchmakingTimedOut:
The request timeout ran out and no match was found.
...
```
This will save the status of each ticket into a database. The client can query his ticketId and keep track of the status of this matchmaking ticket.
If you are using an own webservice based on node.js or similar you can use a websocket connection to forward the notification dirctly to the client. Firebase does not have an sdk for unreal. Thats the reason we are using a database which can be polled by the client. To keep the communication clean we create a endpoint to check the ticket status. The client calls this endpoint and passes all ticketIds it wants to get checked. The enpoint then checks the ticketIds and returns the status. 
 ```
 ...
 * Call StartFlexMatch endoint to start a matchmaking attempt
...
This iterates through all returned tickets, just one in our case and, if the matchmaking was successfull, joins the gamesession.
